class build::php70::extensions::xdebug {
  ...
  bash_exec { 'cd /tmp/xdebug-2.4.0 && phpize-7.3.0':
    require => Bash_exec['cd /tmp && tar xzf xdebug-2.4.0.tgz']
  }

  bash_exec { 'cd /tmp/xdebug-2.4.0 && ./configure --with-php-config=/usr/local/src/phpfarm/inst/bin/php-config-7.3.0':
    timeout => 0,
    require => Bash_exec['cd /tmp/xdebug-2.4.0 && phpize-7.3.0']
  }

  bash_exec { 'cd /tmp/xdebug-2.4.0 && make':
    timeout => 0,
    require => Bash_exec['cd /tmp/xdebug-2.4.0 && ./configure --with-php-config=/usr/local/src/phpfarm/inst/bin/php-config-7.3.0']
  }
