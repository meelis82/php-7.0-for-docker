class build::php70::extensions::memcached {
  ...
  bash_exec { 'cd /tmp/php-memcached-php7 && phpize-7.3.0':
    require => Bash_exec['cd /tmp && unzip php-memcached-php7.zip']
  }

  bash_exec { 'cd /tmp/php-memcached-php7 && ./configure --with-php-config=/usr/local/src/phpfarm/inst/bin/php-config-7.3.0 --enable-memcached-igbinary':
    timeout => 0,
    require => Bash_exec['cd /tmp/php-memcached-php7 && phpize-7.3.0']
  }

  bash_exec { 'cd /tmp/php-memcached-php7 && make':
    timeout => 0,
    require => Bash_exec['cd /tmp/php-memcached-php7 && ./configure --with-php-config=/usr/local/src/phpfarm/inst/bin/php-config-7.3.0 --enable-memcached-igbinary']
  }
