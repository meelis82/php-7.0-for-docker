class build::php70::extensions::igbinary {
  ...
  bash_exec { 'cd /tmp/igbinary7-master && phpize-7.3.0':
    require => Bash_exec['cd /tmp && unzip igbinary7-master.zip']
  }

  bash_exec { 'cd /tmp/igbinary7-master && ./configure --with-php-config=/usr/local/src/phpfarm/inst/bin/php-config-7.3.0 --enable-igbinary':
    timeout => 0,
    require => Bash_exec['cd /tmp/igbinary7-master && phpize-7.3.0']
  }

  bash_exec { 'cd /tmp/igbinary7-master && make':
    timeout => 0,
    require => Bash_exec['cd /tmp/igbinary7-master && ./configure --with-php-config=/usr/local/src/phpfarm/inst/bin/php-config-7.3.0 --enable-igbinary']
  }
